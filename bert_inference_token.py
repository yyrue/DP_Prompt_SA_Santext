...
from utils import word_normalize, get_vocab_SST2, get_vocab_CliniSTS, get_vocab_QNLI
...
def SanText(doc):
    """对单个文档进行清洗
    参数：
        doc:token列表，如["The", "movie", "was", "great"]
    返回：
        new_doc:清洗后的token列表
    ...
    for file_name in ['dev.tsv']:
        ...
        for batch in tqdm(dataloader):
            ...
            with torch.no_grad():
                ...
                #取最大概率的token
                prediction = torch.argmax(prediction,dim=2) # (batch_size, seq_len)
                #提取mask位置的预测（token ID 103 是 [mask]）
                mask_positions = torch.where(batch[0]==103)
                prediction = prediction[mask_positions]

                #与原始token 进行比较（用行索引对齐ground_truths）
                ground_truths = batch[3][mask_positions[0]]
                intersect_num+=(prediction==ground_truths).sum()
                total_num+=len(prediction)


        print(intersect_num.item())#预测正确的数量
        print(total_num)#总数量
        print(1.0*intersect_num.item()/total_num) #攻击成功率
