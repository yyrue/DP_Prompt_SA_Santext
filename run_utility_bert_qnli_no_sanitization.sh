...
# 结果汇总文件
SUMMARY_FILE="$PROJECT_DIR/utility_results_QNLI_bert_no_sanitization.csv"
...
echo "============================================"
echo " BERT Utility 实验 - 无脱敏 Baseline (QNLI)"
...
for RUN in $(seq 1 $NUM_RUNS); do
    ...
    # 微调输出目录
    OUTPUT_FINETUNE="$PROJECT_DIR/output_QNLI_bert_no_sanitization_utility/run_${RUN}"
...
echo "============================================"
echo " 实验说明："
echo " - 本实验直接使用原始 QNLI 数据（无任何脱敏）"
echo " - Accuracy 代表 BERT 微调后在 dev 集上的分类准确率"
echo " - 此结果作为 Utility 的上界（最优参考值）"
echo " - 与 utility_results_QNLI_bert.csv 对比可评估脱敏代价"
echo "============================================"
