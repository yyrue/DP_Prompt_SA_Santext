...
# 结果汇总文件
SUMMARY_FILE="$PROJECT_DIR/utility_results_SST2_bert.csv"
...
echo "============================================"
echo " SanText Utility 实验 (SST-2, BERT Embedding)"
...
for EPSILON in "${EPSILON_LIST[@]}"; do
    ...
    for RUN in $(seq 1 $NUM_RUNS); do
        ...
        # 脱敏输出目录
        OUTPUT_SANITIZE="$PROJECT_DIR/output_SST2_bert_utility/eps_${EPSILON_DIR}/run_${RUN}"
        # 微调输出目录
        OUTPUT_FINETUNE="$PROJECT_DIR/output_SST2_bert_utility/finetune/eps_${EPSILON_DIR}/run_${RUN}"
...
echo "============================================"
echo " 实验说明："
echo " - Accuracy = 在原始dev集上的SST-2分类准确率"
