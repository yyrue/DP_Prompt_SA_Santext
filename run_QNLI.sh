...
SUMMARY_FILE="$PROJECT_DIR/experiment_results_QNLI.csv"
...
for EPSILON in "${EPSILON_LIST[@]}"; do
    ...
    for RUN in $(seq 1 $NUM_RUNS); do
        ...
        # 输出目录
        OUTPUT_SANITIZE="$PROJECT_DIR/output_QNLI/eps_${EPSILON_DIR}/run_${RUN}"
        OUTPUT_FINETUNE="$PROJECT_DIR/output_QNLI/finetune/eps_${EPSILON_DIR}/run_${RUN}"
        ...
        # ---- 断点续跑：检查该轮是否已完成 ----
        EVAL_FILE="$OUTPUT_FINETUNE/eval_results_qnli.txt"
        if [ -f "$EVAL_FILE" ]; then
            echo "⏭️  已完成，跳过（结果文件已存在）"
            continue
        fi

        # ============================================================
        # 阶段一：文本脱敏（若脱敏数据已存在则跳过）
        # ============================================================
        if [ -f "$ACTUAL_SANITIZE_DIR/train.tsv" ]; then
            echo ""
            echo "[阶段 1/2] 脱敏数据已存在，跳过脱敏..."
        else
            echo ""
        ...
        # 提取准确率（QNLI 结果文件为 eval_results_qnli.txt）
