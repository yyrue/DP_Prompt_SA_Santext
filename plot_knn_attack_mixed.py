...
# -*- coding: utf-8 -*-
"""
绘制 KNN Attack (Top-1) Defense Rate 曲线
横坐标：ε (MLDP)，标注实际隐私预算为 ε·d_max
纵坐标：Defense Rate
五条曲线：SanText baseline + eps_high = 16, 18, 20, 24
"""
...
for eps_high in EPS_HIGH_LIST:
    csv_path = f"{PROJECT_DIR}/knn_attack_results_SST2_mixed_eps{eps_high}_Top{TOPK}.csv"
    data = {}
    try:
...
santext_csv = f"{PROJECT_DIR}/knn_attack_results_SST2_v2_Top{TOPK}.csv"
santext_data = {}
try:
...
ax.set_title(f'KNN Attack (Top-{TOPK}) Defense Rate — SST2, Sample Amplification', fontsize=15)
...
# ---- 保存 ----
out_png = f"{PROJECT_DIR}/knn_attack_defense_rate_mixed_Top{TOPK}.png"
out_pdf = f"{PROJECT_DIR}/knn_attack_defense_rate_mixed_Top{TOPK}.pdf"
