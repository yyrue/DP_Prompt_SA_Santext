...
#
# 用法:
#   bash run_utility_bert_qnli_pretokenized.sh
# ============================================================
...
# 预分词数据输出目录（不会修改原始数据）
PRETOKENIZED_DIR="$PROJECT_DIR/output_QNLI_bert_pretokenized"

# 每个实验跑几轮
NUM_RUNS=5

# 结果汇总文件
SUMMARY_FILE="$PROJECT_DIR/utility_results_QNLI_bert_pretokenized.csv"
...
for RUN in $(seq 1 $NUM_RUNS); do
    ...
    # 微调输出目录
    OUTPUT_FINETUNE="$PROJECT_DIR/output_QNLI_bert_pretokenized_utility/run_${RUN}"
...
echo "============================================"
echo " 实验说明："
echo " - 本实验使用经过 BertTokenizer.tokenize() 预分词的 QNLI 数据"
...
echo " - 对比文件："
echo "     无脱敏(未分词): utility_results_QNLI_bert_no_sanitization.csv"
echo "     无脱敏(预分词): utility_results_QNLI_bert_pretokenized.csv"
echo "     脱敏版本:       utility_results_QNLI_bert.csv"
echo "============================================"
