#!/bin/bash
# ============================================================
# Mask Token Inference Attack 实验脚本 (直接脱敏数据版本)
# 数据集: QNLI
# 流程: 读取直接脱敏数据 → Mask Token Attack → 统计防御率
# 数据来源: output_QNLI_bert_utility/eps_X.XX/run_Y/eps_X.XX/
# ============================================================
...
DATA_ROOT="$PROJECT_DIR/output_QNLI_bert_utility"  # 直接脱敏数据根目录

# ============================================================
# epsilon 列表（与脱敏数据目录对应）
# ============================================================
EPSILON_LIST=(12.5 13.5 15 15.5)

# 每个 epsilon 跑几轮
NUM_RUNS=5

# 结果汇总文件
SUMMARY_FILE="$PROJECT_DIR/mask_attack_results_QNLI.csv"
if [ ! -f "$SUMMARY_FILE" ]; then
    echo "epsilon,run,seed,defense_rate,attack_success_rate" > "$SUMMARY_FILE"
fi

# 攻击结果输出根目录
ATTACK_OUTPUT_ROOT="$DATA_ROOT/attack"
...
echo "============================================"
echo " Mask Token Inference Attack (QNLI, 直接脱敏数据)"
...
for EPSILON in "${EPSILON_LIST[@]}"; do
    ...
    for RUN in $(seq 1 $NUM_RUNS); do
        ...
        # 脱敏数据目录
        SANITIZED_DIR="$DATA_ROOT/eps_${EPSILON_DIR}/run_${RUN}/eps_${EPSILON_DIR}"
...
echo " - 数据来源: output_QNLI_bert_utility/（直接脱敏数据）"
echo "============================================"
