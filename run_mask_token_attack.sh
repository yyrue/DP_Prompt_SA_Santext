#!/bin/bash
# ============================================================
# Mask Token Inference Attack 实验脚本
# 数据集: SST-2
# 使用 BERT embedding 进行文本清洗
# 然后使用预训练的 BERT MLM 进行攻击测试
# ============================================================
...
TASK="SST-2"
METHOD="SanText"
EMBEDDING="bert"  # 使用 BERT embedding（重要！）

# ============================================================
# epsilon 列表：论文中 Figure 4 使用的值
# 论文 Table 3 显示：12, 14, 16, 18
# ============================================================
...
# 结果汇总文件
SUMMARY_FILE="$PROJECT_DIR/mask_attack_results_SST2.csv"
...
for EPSILON in "${EPSILON_LIST[@]}"; do
    ...
    for RUN in $(seq 1 $NUM_RUNS); do
        ...
        # 输出目录（用于保存清洗后的文本）
        OUTPUT_SANITIZE="$PROJECT_DIR/output_SST2_bert/eps_${EPSILON_DIR}/run_${RUN}"
        ...
        # ============================================================
        # 阶段二：Mask Token Inference Attack
        # ============================================================
        echo ""
        ...
        # 从输出中提取攻击成功率和防御率
        # bert_inference_token.py 输出格式：
        # intersect_num (预测正确的数量)
        # total_num (总数量)
        # ratio (攻击成功率)
