...
# -*- coding: utf-8 -*-
"""
KNN Attack v2 —— 直接读取已有脱敏文件，跳过脱敏步骤

与 v1 的区别：
  v1: 加载 embedding → 计算概率矩阵 → SanText 脱敏 → KNN Attack
  v2: 直接读取已有 sanitized dev.tsv → KNN Attack  （省去概率矩阵计算，速度大幅提升）
...
def read_sst2_tsv(tsv_path, tokenizer):
    """读取 SST-2 格式的 tsv，返回 token 列表的列表"""
    docs = []
    with open(tsv_path, "r") as f:
        ...
        for line in f:
            ...
            if len(parts) < 1:
                continue
            text = parts[0]
            # 脱敏文件中 token 已经是空格分隔的 subword，直接 split 即可
            # 原始文件需要重新 tokenize
            doc = text.split()
            docs.append(doc)
    return docs
...
def read_sst2_tsv_tokenize(tsv_path, tokenizer):
    """读取原始 SST-2 tsv，用 BERT tokenizer 分词"""
    docs = []
    with open(tsv_path, "r") as f:
        next(f)
        for line in f:
...
def main():
    parser = argparse.ArgumentParser(
        description="KNN Attack v2 —— 直接读取已有脱敏文件，跳过脱敏步骤"
    )

    parser.add_argument("--bert_model_path",  type=str, default="./bert-base-uncased")
    parser.add_argument("--original_data_dir",type=str, default="./data/SST-2",
                        help="原始数据目录（含 dev.tsv）")
    parser.add_argument("--sanitized_dir",    type=str, required=True,
                        help="脱敏后数据目录（含 dev.tsv），如 output_SST2_bert_utility/eps_2.00/run_1/eps_2.00")
    parser.add_argument("--result_file",      type=str, default="./knn_attack_results_SST2_v2.csv")
    ...
    logger.info("=" * 60)
    logger.info(f"KNN Attack v2 | ε={args.epsilon} | run={args.run} | "
    ...
    all_word_embed = np.array(all_word_embed, dtype=np.float32)  # (30522, 768)
    ...
    original_docs  = read_sst2_tsv_tokenize(orig_path, tokenizer)
    # 脱敏文件：token 已经是 subword，直接 split
    sanitized_docs = read_sst2_tsv(san_path, tokenizer)

    logger.info(f"  原始文档数: {len(original_docs)}")
    logger.info(f"  脱敏文档数: {len(sanitized_docs)}")

    # 对齐长度（脱敏文件可能有截断）
    assert len(original_docs) == len(sanitized_docs), \
        f"文档数量不一致: orig={len(original_docs)}, san={len(sanitized_docs)}"

    # 对齐每条文档的 token 数（脱敏文件可能因截断导致长度不同）
    aligned_orig = []
    aligned_san  = []
    for orig_doc, san_doc in zip(original_docs, sanitized_docs):
