...
# -*- coding: utf-8 -*-
"""
绘制 Defense Rate 对比图（Mask Token Inference Attack）— QNLI 数据集
- X 轴: epsilon
- Y 轴: Defense Rate
- 对比线:
  1. Baseline（原始算法）
...
def load_avg(csv_path, key_col="epsilon", val_col="defense_rate"):
    """按 epsilon 分组，计算 defense_rate 的均值和标准差"""
...
baseline_csv = os.path.join(PROJECT_DIR, "mask_attack_results_QNLI.csv")
mixed_csvs = {
    "Mixed (ε_high=16)": os.path.join(PROJECT_DIR, "mask_attack_results_QNLI_mixed_eps16.csv"),
    "Mixed (ε_high=18)": os.path.join(PROJECT_DIR, "mask_attack_results_QNLI_mixed_eps18.csv"),
    "Mixed (ε_high=20)": os.path.join(PROJECT_DIR, "mask_attack_results_QNLI_mixed_eps20.csv"),
    "Mixed (ε_high=22)": os.path.join(PROJECT_DIR, "mask_attack_results_QNLI_mixed_eps22.csv"),
    "Mixed (ε_high=24)": os.path.join(PROJECT_DIR, "mask_attack_results_QNLI_mixed_eps24.csv"),
}
...
baseline_epsilons = sorted(baseline_mean.keys())
...
ax.set_title("Privacy Comparison: Baseline vs Mixed Sampling — Mask Token Attack (QNLI)", fontsize=15)
...
# 7. 保存图片
# ============================================================
output_png = os.path.join(PROJECT_DIR, "defense_rate_comparison_QNLI_mask_token_attack.png")
output_pdf = os.path.join(PROJECT_DIR, "defense_rate_comparison_QNLI_mask_token_attack.pdf")
