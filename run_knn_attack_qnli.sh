#!/bin/bash
# ============================================================
# KNN Attack 运行脚本 (QNLI, 直接脱敏数据)
# 直接读取已有脱敏文件，跳过脱敏步骤
#
# 脱敏文件路径：
#   output_QNLI_bert_utility/eps_{EPS}/run_{RUN}/eps_{EPS}/dev.tsv
# ============================================================
...
SANITIZED_BASE="$PROJECT_DIR/output_QNLI_bert_utility"
...
# ---- 结果文件 ----
RESULT_FILE="$PROJECT_DIR/knn_attack_results_QNLI_Top${TOPK}.csv"
...
for EPSILON in "${EPSILON_LIST[@]}"; do
    ...
    for RUN in $(seq 1 $NUM_RUNS); do
        ...
        if [ ! -f "$SANITIZED_DIR/dev.tsv" ]; then
            echo "  文件不存在，跳过: $SANITIZED_DIR/dev.tsv"
            continue
        fi
...
with open('$RESULT_FILE') as f:
    ...
    for row in reader:
        if abs(float(row['epsilon']) - $EPSILON) < 0.001 and int(row['run']) == $RUN and int(row['topk']) == $TOPK:
            print('found')
            break
" 2>/dev/null)
...
if __import__('os').path.exists("$RESULT_FILE"):
    with open("$RESULT_FILE") as f:
        reader = csv.DictReader(f)
        for row in reader:
            if abs(float(row['epsilon']) - $EPSILON) < 0.001:
                try:
...
print(f"\n  KNN Attack (Top-$TOPK) 结果汇总 — QNLI, BERT Embedding")
